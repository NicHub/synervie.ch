...
/**
 * Injecte le schéma JSON-LD de Synervie dans le <head>
 */
function synervie_add_jsonld_schema() {
	$jsonld_file = get_template_directory() . '/test/synervie.jsonld';
	if ( file_exists( $jsonld_file ) ) {
		$jsonld = file_get_contents( $jsonld_file );
		echo '<script type="application/ld+json">' . $jsonld . '</script>';
	}
}
add_action( 'wp_head', 'synervie_add_jsonld_schema' );
